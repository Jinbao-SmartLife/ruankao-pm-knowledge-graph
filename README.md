...
软考（信息系统项目管理师）交互式知识图谱，涵盖十大管理 ITTO、八大绩效域、论文真题、记忆口诀，辅助备考记忆与复习。共 4 个页面。

**在线访问**：[https://ruankao-pm.smrthomehub.com/](https://ruankao-pm.smrthomehub.com/)
...
八大绩效域专项记忆页面，含检查表、核心要点、对比分析，辅助理解与背诵。自包含页面（内联 CSS/JS），不依赖外部样式表。

### 关联记忆 (`memory.html`)

知识点关联记忆页面，通过流程链、工具共享图、模式卡片等可视化方式强化记忆。自包含页面（内联 CSS/JS），不依赖外部样式表。
...
- 暗色主题（GitHub Dark 风格）
- 响应式布局，适配桌面与移动端
  - 900px：中等屏幕适配（grid 列数调整）
  - 768px：移动端适配（body 滚动、单列布局、导航横滑、底部抽屉面板）
  - 480px：小屏幕适配（进一步缩减间距与字号）
