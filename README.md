...
软考（信息系统项目管理师）十大管理 ITTO 交互式知识图谱，辅助备考记忆与复习。
...
| 视图 | 说明 |
|------|------|
| **矩阵视图** | 10 大知识领域 × 5 大过程组完整矩阵，含 49 个过程，考频热力标识 |
...
- **左上**：紧凑型 PMBOK 矩阵，点击/悬停选择过程
- **左下**：所选过程的输入/工具/输出三栏展示 + 论文写作素材
- **右上**：全部文档按 13 类分组（项目章程、PM 计划、基准等），可折叠
- **右下**：全部工具按 11 类分组（专家判断、数据收集、数据分析等），可折叠

支持跨面板联动高亮：选择过程时，右侧面板自动标识相关文档/工具的角色。

## 数据内容

| 数据文件 | 内容 |
|----------|------|
| `nodes.js` | ~420 个节点（知识领域、绩效域、过程组、过程、文档、工具） |
| `edges.js` | 图谱边（consumed-by / produces / used-in 三种关系） |
| `exam-frequency.js` | 各知识领域和绩效域的考频、高频过程列表 |
| `mnemonics.js` | 36 条记忆口诀 |
| `essay-topics.js` | 19 道论文真题（2015-2025），含要点和难度评级 |
| `glossary.js` | 68 个项目管理术语释义 |
| `itto-details.js` | 49 个过程的论文写作模板（问题-做法-效果-框架） |
